...
if (!empty($entradas)) {
	...
	foreach($entradas as $key => $m) {
		...
		if (!empty($salidas[$key][0][0]['SUM(`Inventarioinsumo`.`cantidad`)'])) {
			$salida1 = $salidas[$key][0][0]['SUM(`Inventarioinsumo`.`cantidad`)'];
		...
		if (!empty($entradas[$key-1])) {
			...
			if (!empty($entradas[$key-1][0][0]['SUM(`Inventarioinsumo`.`cantidad`)'])) {
				$e_inicial = $entradas[$key-1][0][0]['SUM(`Inventarioinsumo`.`cantidad`)'];
			} else {
				$e_inicial = 0;
			}
			if (!empty($salidas[$key-1][0][0]['SUM(`Inventarioinsumo`.`cantidad`)'])) {
				$s_inicial = $salidas[$key-1][0][0]['SUM(`Inventarioinsumo`.`cantidad`)'];
			} else {
				$s_inicial = 0;
			}
		...
		if ($m[0]['Inventarioinsumo']['trimestre'] == $trimestre) {
		...
		echo '<tr>';
		echo '<td>'.$m[0]['Inventarioinsumo']['trimestre'].'</td>';
